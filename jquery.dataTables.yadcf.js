...
* create_row
                ...
                Type:               boolean
                Default value:      undefined
                Description:        Create <tr> for filters inside the thead of the table
...
(function (root, factory) {
    ...
    var yadcf = (function () {
        ...
        function addTriStateFilter(filter_selector_string, table_selector_jq_friendly, column_number) {
            var filter_wrapper_id,
                oTable,
                columnObj,
                filterActionStr;
            ...
            filterActionStr = 'onchange="yadcf.triStateClick(\'' + table_selector_jq_friendly + '\', event);"';
            if (columnObj.externally_triggered === true) {
                filterActionStr = '';
            }

            let $filterWrapper = $('<div />').addClass('tri-state-box').data('column-number', column_number);
            let randPart = Math.floor(Math.random() * 10000);

            $filterWrapper
                .append($('<input type="radio" ' + filterActionStr + ' name="tribox_'+ randPart +'" id="id_state_false_' + randPart + '" value="-1" />'))
                .append($('<label class="state-false" title="' + columnObj.filter_settings['false'].title + '" for="id_state_false_' + randPart + '" />'))

                .append($('<input type="radio" ' + filterActionStr + ' name="tribox_'+ randPart +'" id="id_state_indeterminate_' + randPart + '" value="0" />'))
                .append($('<label class="state-indeterminate" title="' + columnObj.filter_settings['indeterminate'].title + '" for="id_state_indeterminate_' + randPart + '" />'))

                .append($('<input type="radio" ' + filterActionStr + ' name="tribox_'+ randPart +'" id="id_state_true_' + randPart + '" value="1" />'))
                .append($('<label class="state-true" title="' + columnObj.filter_settings['true'].title + '" for="id_state_true_' + randPart + '" />'))
            ...
            try {
                if (
                    false != oTable &&
                    false != oTable.fnSettings() &&
                    false != oTable.fnSettings().oLoadedState &&
                    false != oTable.fnSettings().oLoadedState.columns &&
                    false != oTable.fnSettings().oLoadedState.columns[column_number] &&
                    false != oTable.fnSettings().oLoadedState.columns[column_number].search &&
                    false != oTable.fnSettings().oLoadedState.columns[column_number].search.search
                ) {
                    search = oTable.fnSettings().oLoadedState.columns[column_number].search.search;
                }
            } catch(exception) {
                // no loaded state - fallback to default
                console.log(exception);
                search = '';
            }

            if (search === columnObj.filter_settings['false'].value) {
                $filterWrapper.find('#id_state_false_' + randPart).prop('checked', true);
            } else if (search === columnObj.filter_settings['true'].value) {
                $filterWrapper.find('#id_state_true_' + randPart).prop('checked', true);
            } else {
                $filterWrapper.find('#id_state_indeterminate_' + randPart).prop('checked', true);
            }

            $(filter_selector_string).append($filterWrapper);
		...
		*/
            ...
            for (columnObjKey in args) {
                if (args.hasOwnProperty(columnObjKey)) {
                    ...
                    if ($filter_selector.length === 1) {
                        ...
                        } else if (columnObj.filter_type === "tri_state") {
                            addTriStateFilter(filter_selector_string, table_selector_jq_friendly, column_number);
                        }
                    }
        ...
        function triStateClick(table_selector_jq_friendly, event) {
            var oTable,
                state,
                filterId,
                column_number,
            ...
            if (false === $(event.target).prop('checked')) {
                return;
            }

            filterId = event.target.id;
            state = document.getElementById(filterId).value - 0;

            $.fn.dataTableExt.iApiIndex = oTablesIndex[table_selector_jq_friendly];
            oTable = oTables[table_selector_jq_friendly];
            column_number = $(event.target).parent().data('column-number') - 0;
            columnObj = getOptions(oTable.selector)[column_number];

            switch (state) {
                case -1:
                    filterValue = columnObj.filter_settings['false'].value;
                    break;
                case 1:
                    filterValue = columnObj.filter_settings['true'].value;
                    break;
                default:
                    filterValue = '';
                    break
            ...
            oTable.fnFilter(filterValue, column_number);
            resetIApiIndex();
        }
	...
	*/
        ...
        function init(oTable, options_arg, params) {
            ...
            if (params.filters_position === undefined || params.filters_position === 'header') {
                if (params.create_row) {
                    let $thead = $(instance[0]).find('thead').length ? $(instance[0]).find('thead') : $('<thead />');

                    let count = oTable.columns()[0].length;
                    let $tr = $('<tr />');
